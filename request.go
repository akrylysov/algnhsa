...
func newLambdaRequest(ctx context.Context, payload []byte, opts *Options) (lambdaRequest, error) {
	...
	case RequestTypeAPIGatewayWebsocket:
		return newAPIGatewayWebsocketRequest(ctx, payload, opts)
	case RequestTypeALB:
		return newALBRequest(ctx, payload, opts)
	}

	// The request type wasn't specified.
	// Try to decode the payload as APIGatewayProxyRequest, then APIGatewayWebsocketProxyRequest, if
	// it fails try ALBTargetGroupRequest.
	...
	req, err = newAPIGatewayWebsocketRequest(ctx, payload, opts)
	if err != nil && err != errAPIGatewayWebsocketUnexpectedRequest {
		return lambdaRequest{}, err
	}
	if err == nil {
		return req, nil
